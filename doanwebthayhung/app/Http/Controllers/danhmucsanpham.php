...
class danhmucsanpham extends Controller
{
    ...
    public function postsua($MaSP, Request $request){
        ...
        /* echo $request->hinhanh;
        echo strstr( $request->hinhanh, '.');
        exit; */
        if(strstr( $request->hinhanh, '.')!='.jpg' && strstr( $request->hinhanh, '.')!='.png'){
            return redirect('addsanpham')->with('thongbaokhongthanhcong','đuôi file phải là .jpg hoặc .png');
        }
        /* exit; */
        /* kiểm tra input file có rỗng ko nếu rỗng gán giá trị ảnh cũ vào */
